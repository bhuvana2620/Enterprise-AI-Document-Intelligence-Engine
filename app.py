...
def resolve_api_base_url() -> str:
    ...
    In this deployment, FastAPI and Streamlit run as two processes inside
    the same Hugging Face Spaces Docker container, started by start.sh.
    The backend always listens on localhost, so no cross-host networking
    or platform-specific hostport injection is required.

    Override with API_BASE_URL for local development against a backend
